...
try:
    ...
    print ('Use op_infer')
    from .op_infer import (FusedLeakyReLU, fused_leaky_relu, upfirdn2d) #my~
...
class ToFlow(nn.Module):
    ...
    def forward(self, h, style, feat, skip=None):
        ...
        feat_warp = F.grid_sample(feat.float(), flow.float(), align_corners=True).half() * mask
...
class Direction(nn.Module):
    ...
    def forward(self, input):
        ...
        Q, R = torch.linalg.qr(weight.float())    # get eignvector, orthogonal [n1, n2, n3, n4]

        input_diag = torch.diag_embed(input)  # alpha, diagonal matrix
        out = torch.matmul(input_diag, Q.T.half())
